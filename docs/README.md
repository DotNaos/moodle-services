# Docs

Use this folder when you already know what you want to do.

## Guides

- [Run the API with Docker and a saved session](guides/run-api-with-docker-saved-session.md)
- [Bootstrap a server with one command](guides/bootstrap-server.md)
- [Run the API with Docker and a fresh login](guides/run-api-with-docker-throwaway.md)
- [Run the API with Docker Compose](guides/run-api-with-docker-compose.md)
- [Docker OCR](guides/ocr-docker.md)
- [Use the CLI locally](guides/install.md)
- [Run the API locally](guides/run-api-locally.md)
- [Update the CLI](guides/update.md)

## Reference

- [CLI commands](reference/cli.md)
- [API endpoints](reference/api.md)
- [Default paths and environment variables](reference/paths.md)

## Troubleshooting

- [Login and config problems](troubleshooting/login-and-config.md)
- [Docker session and credential problems](troubleshooting/docker-sessions.md)

## Development

- [Moodle Services roadmap](development/moodle-services-roadmap.md)
- [Internals](development/internals.md)
- [Architecture](development/architecture.md)
- [Code structure](development/code-structure.md)
- [Release workflow](development/release-workflow.md)
