...
func TestBuildMobileLaunchURL(t *testing.T) {
	launch, err := BuildMobileLaunchURL("https://moodle.fhgr.ch/", DefaultMobileLaunchService, "passport-123", "studyreplay")
	if err != nil {
...
func TestParseMobileLaunchCallbackRejectsInvalidBase64(t *testing.T) {
	_, err := ParseMobileLaunchCallback("studyreplay://token=not-valid-!!!")
	if err == nil {
		t.Fatal("expected invalid base64 to fail")
	}
}

func TestMobileTokenFromLaunch(t *testing.T) {
	token := MobileTokenFromLaunch("https://moodle.fhgr.ch/", MobileLaunchToken{
		Token:        "public-token",
		PrivateToken: "private-token",
	})
	if token.SiteURL != "https://moodle.fhgr.ch" {
		t.Fatalf("unexpected site URL %q", token.SiteURL)
	}
	if token.Token != "public-token" || token.PrivateToken != "private-token" {
