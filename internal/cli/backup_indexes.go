...
func uploadExportNavigation(ctx context.Context, uploader backupDriveUploader, run backupRunContext, courses []backupCourse, records []exportMaterialRecord) error {
	...
	if _, err := uploader.UploadText(ctx, semesterBriefing(run.Semester, run.RunID, courses, records), currentFolder.ID, "semester.briefing.md", true); err != nil {
		return err
	}
	index := buildSemesterIndex(run, courses, records)
